...
function get_module_settings_part_heading($pagebuilder, $moduleid, $pb_module_size_now)
{
    if (!isset($pagebuilder['modules'][$moduleid]['heading_text'])) {
        $pagebuilder['modules'][$moduleid]['heading_text'] = '';
    }
    if (!isset($pagebuilder['modules'][$moduleid]['heading_alignment'])) {
        $pagebuilder['modules'][$moduleid]['heading_alignment'] = '';
    }
    ...
    <div class='padding-cont'>
        ...
        <div class='alignment'>
            ...
            <select class='newselect' name='pagebuilder[modules][$moduleid][heading_alignment]'>
                " . get_select_options($GLOBALS["pbconfig"]['all_available_headers_alignment'], $pagebuilder['modules'][$moduleid]['heading_alignment']) . "
            </select>
        </div>
...
function get_module_settings_part_textarea($pagebuilder, $moduleid, $pb_module_size_now, $temp, $headingtext, $settingname, $tinymce_activation_class = "")
{
    return "
        <div class='enter_option_row tinyCont'>
            <h5>{$headingtext}</h5>
                <textarea name='pagebuilder[modules][$moduleid][$settingname]' class='enter_text1' id='" . $tinymce_activation_class . "'>" . (isset($pagebuilder['modules'][$moduleid][$settingname]) ? $pagebuilder['modules'][$moduleid][$settingname] : '') . "</textarea>
        </div>
    ";
}

function get_module_settings_part_textarea_without_tiny($pagebuilder, $moduleid, $pb_module_size_now, $temp, $headingtext, $settingname, $tinymce_activation_class = "")
{
    return "
...
function get_module_settings_part_select($pagebuilder, $moduleid, $pb_module_size_now, $temp, $headingtext, $settingname, $width, $available_options_in_select)
{
    return "
        <div class='enter_option_row'>
            <h5>$headingtext</h5>
...
function get_module_settings_part_select_with_caption($pagebuilder, $moduleid, $pb_module_size_now, $temp, $headingtext, $settingname, $width, $available_options_in_select)
{
    return "
        <div class='enter_option_row'>
            <h5>$headingtext</h5>
...
function get_module_settings_part_upload_file($pagebuilder, $moduleid, $pb_module_size_now, $temp, $headingtext, $settingname, $width)
{
    return "
...
function get_module_settings_part_input($pagebuilder, $moduleid, $pb_module_size_now, $temp, $headingtext, $settingname, $width, $textalign, $defaultvalue = '')
{
    return "
        <div class='enter_option_row'>
            <h5>$headingtext</h5>
            <input style='width:{$width} !important; text-align: {$textalign} !important;' type='text' value='" . ((isset($pagebuilder['modules'][$moduleid][$settingname]) && strlen($pagebuilder['modules'][$moduleid][$settingname]) > 0) ? $pagebuilder['modules'][$moduleid][$settingname] : $defaultvalue) . "' name='pagebuilder[modules][$moduleid][$settingname]' class='enter_text1'>
        </div>
    ";
}


function get_module_settings_part_cpts($pagebuilder, $moduleid, $headingtext, $settingname, $cpt)
{
    $compile = "
    <div class='enter_option_row'>
        <h5>$headingtext</h5>";
    ...
    if (count($wp_query_cpt_posts->query($args)) > 0) {
        ...
        while ($wp_query_cpt_posts->have_posts()) : $wp_query_cpt_posts->the_post();
            ...
            if (isset($pagebuilder['modules'][$moduleid][$settingname][get_the_ID()]) && $pagebuilder['modules'][$moduleid][$settingname][get_the_ID()] == "on") {
...
function get_module_settings_part_categories($pagebuilder, $moduleid, $headingtext, $settingname, $cpt)
{
    $rnd = mt_rand(1, 9999);
    $checked_isset = "checked";
    $cathided_state = "cat_hided";
    if (isset($pagebuilder['modules'][$moduleid]['show_all_categories']) && $pagebuilder['modules'][$moduleid]['show_all_categories'] == "on") {
        $cathided_state = "cat_hided";
    }
    $compile = "
    <div class='enter_option_row'>
    ...
    if ($cpt == "post") {
        ...
        if (count($categories) > 0) {
            ...
            foreach ($categories as $cat) {
                if (isset($pagebuilder['modules'][$moduleid][$settingname][$cat->cat_ID]) && $pagebuilder['modules'][$moduleid][$settingname][$cat->cat_ID] == "on") {
                    $checked_isset = "";
                    $cathided_state = "";
                    continue;
                }
            ...
            $compile .= "<input class='all_part' " . $checked_isset . " " . ((isset($pagebuilder['modules'][$moduleid]['show_all_categories']) && $pagebuilder['modules'][$moduleid]['show_all_categories'] == "on") ? "checked" : "") . " type='checkbox' name='pagebuilder[modules][$moduleid][show_all_categories]'> <span>" . __("All", "gt3_builder") . "</span>";
        ...
        if (is_array($terms) && count($terms) > 0) {
            ...
            foreach ($terms as $cat) {
                if (isset($pagebuilder['modules'][$moduleid][$settingname][$cat->term_id]) && $pagebuilder['modules'][$moduleid][$settingname][$cat->term_id] == "on") {
                    $checked_isset = "";
                    $cathided_state = "";
                    continue;
                }
...
function get_module_settings_part_icons($pagebuilder, $moduleid, $headingtext, $settingname)
{
    $compile = "
        <div class='enter_option_row all_available_font_icons'>
            <h5>" . $headingtext . "</h5>
    ...
    $compile .= "
            ...
            <div class='clear'></div>
            <input type='text' value='" . (isset($pagebuilder['modules'][$moduleid][$settingname]) ? $pagebuilder['modules'][$moduleid][$settingname] : '') . "' name='pagebuilder[modules][$moduleid][$settingname]' class='enter_text1 icon_type'>
        </div>
    ";
...
function get_module_settings_part_any_icons($pagebuilder, $moduleid, $headingtext, $settingname)
{
    $compile = "
    ...
    if (isset($pagebuilder['modules'][$moduleid][$settingname]) && is_array($pagebuilder['modules'][$moduleid][$settingname])) {
        foreach ($pagebuilder['modules'][$moduleid][$settingname] as $key => $value) {
            $compile .= "
...
function get_module_settings_part_padding_bottom($pagebuilder, $moduleid)
{
    $compile = "
        <div class='enter_option_row'>
            <h5>" . __('You can use the custom CSS class for this module.', 'gt3_builder') . "</h5>
            <input type='text' value='" . (isset($pagebuilder['modules'][$moduleid]['custom_class']) ? $pagebuilder['modules'][$moduleid]['custom_class'] : '') . "' name='pagebuilder[modules][$moduleid][custom_class]' class='enter_text1' style='width:115px !important; text-align:center;'>
        </div>
        <div class='enter_option_row margin-bottom-block'>
            <h5>" . __('Please set the custom margin-bottom of the module.', 'gt3_builder') . "</h5>
            <input type='text' value='" . (isset($pagebuilder['modules'][$moduleid]['padding_bottom']) ? $pagebuilder['modules'][$moduleid]['padding_bottom'] : $GLOBALS["pbconfig"]['default_padding_after_module']) . "' name='pagebuilder[modules][$moduleid][padding_bottom]' class='enter_text1' style='width:115px !important; text-align:center;'>
        </div>
    ";
...
function get_module_settings_part_add_list_ability($pagebuilder, $moduleid, $pb_module_size_now, $temp, $caption)
{
    if (!isset($compile)) {
        $compile = '';
    }
