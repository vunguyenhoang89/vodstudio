...
{
	if (isset($gt3_theme_pagebuilder['modules'][$moduleid][$settingname]) && strlen($gt3_theme_pagebuilder['modules'][$moduleid][$settingname]) > 0) {
		$gt3_current_val = $gt3_theme_pagebuilder['modules'][$moduleid][$settingname];
	} else {
		$gt3_current_val = $defaultvalue;
	}
    return "
        <div class='enter_option_row'>
            <h5>$headingtext</h5>
            <input style='width:{$width} !important; text-align: {$textalign} !important;' type='text' value='" . htmlspecialchars($gt3_current_val,ENT_QUOTES) . "' name='pagebuilder[modules][$moduleid][$settingname]' class='enter_text1'>
        </div>
    ";
}
