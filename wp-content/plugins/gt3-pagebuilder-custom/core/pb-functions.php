...
{
    if (!isset($compile)) {
        $compile = '<ul class="sortable-img-items gt3_post_images">';
    }
        ...
        foreach ($gt3_theme_pagebuilder['post-formats']['images'] as $imgid => $img) {
			...
			<div class='img-item style_small'>
				<div class='img-preview'>
					<img src='" . aq_resize(wp_get_attachment_url($img['attach_id']), "156", "106", true) . "' data-full-url='" . wp_get_attachment_url($img['attach_id']) . "' data-thumb-url='" . aq_resize(wp_get_attachment_url($img['attach_id']), "156", "106", true, true, true) . "' alt='' class='previmg'>
					<div class='hover-container gt3_hover-container'>
						<div class='inter_x'></div>
						<div class='inter_drag'></div>
					</div>
				</div>
				<input type='hidden' name='pagebuilder[post-formats][images][" . $imgid . "][attach_id]' value='{$img['attach_id']}'>
			</div>
			</li>
			";
        }
...
if (!function_exists('showAsidePortCats')) {
    function showAsidePortCats($post_type_terms = "")
    {
        if (!isset($term_list)) {
            $term_list = '';
        }
        ...
        if ($count > 0) {
            ...
            if ($count > 1) {
                ...
                $term_list .= '<a href="' . $permalink . '" class="show_all" data-option-value="*">' . __('All Works', 'gt3_builder') . '</a>
			</li>';
            }
            $termcount = count($terms);
            if (is_array($terms)) {
...
if (!function_exists('showAsideGalleryCats')) {
    function showAsideGalleryCats($post_type_terms = "")
    {
        if (!isset($term_list)) {
            $term_list = '';
        }
