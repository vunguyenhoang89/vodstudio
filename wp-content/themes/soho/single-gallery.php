		...
		foreach ($gt3_theme_pagebuilder['sliders']['fullscreen']['slides'] as $imageid => $image) {
			...
			if ($image['slide_type'] == 'image') {
				...
				if ($is_youtube > 0) {
					...
					$sliderCompile .= '{type: "youtube", uniqid: "' . $uniqid . '", src: "' . $videoid . '", thmb: "", title: "' . str_replace('"', "'", $photoTitle) . '", description: "' . str_replace('"', "'",  $photoCaption) . '", titleColor: "#'.$titleColor.'", descriptionColor: "#'.$captionColor.'"},';
				}
				#VIMEO
				...
				if ($is_vimeo > 0) {
					...
					$sliderCompile .= '{type: "vimeo", uniqid: "' . $uniqid . '", src: "' . $videoid . '", thmb: "", title: "' . str_replace('"', "'", $photoTitle) . '", description: "' . str_replace('"', "'",  $photoCaption) . '", titleColor: "#'.$titleColor.'", descriptionColor: "#'.$captionColor.'"},';
                    ...
                    <div <?php post_class("blogpost_preview_fw"); ?>>
                        <div class="fw-portPreview">
                            <div class="img_block wrapped_img fs_port_item gallery_item_wrapper">
                                <?php
									if ($image['slide_type'] == "image") {
										echo '<a class="featured_ico_link swipebox" rel="gallery-post" href="'. wp_get_attachment_url($image['attach_id']) .'" title="'.$photoTitle.'">';
									} else {
										$set_rel = '';
										$is_youtube = substr_count($image['src'], "youtu");	
										if ($is_youtube > 0) {
											$set_rel = 'gallery-post';
										}
										$is_vimeo = substr_count($image['src'], "vimeo");
										if ($is_vimeo > 0) {
											$set_rel = 'gallery-post';
